...
class ControllerStartupStartup extends Controller {
	...
	if (isset($this->request->get['_route_'])) {
		...
		if (isset($url['path'])) {
			...
			$server = $_SERVER['HTTP_HOST'] . '/' . $store;

			if (isset($this->request->server['HTTPS']) && (($this->request->server['HTTPS'] == 'on') || ($this->request->server['HTTPS'] == '1'))) {
				$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "store WHERE REPLACE(`ssl`, 'www.', '') = '" . $this->db->escape('https://' . str_replace('www.', '', $server) . rtrim(dirname($_SERVER['PHP_SELF']), '/.\\') . '/') . "'");
			} else {
				$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "store WHERE REPLACE(`url`, 'www.', '') = '" . $this->db->escape('http://' . str_replace('www.', '', $server) . rtrim(dirname($_SERVER['PHP_SELF']), '/.\\') . '/') . "'");
			}
			if ($query->num_rows) {
				$store = $store . '/';
				$this->request->get['_route_'] = str_replace($store, '',$this->request->get['_route_']);
				$this->session->data['store_id'] = $query->row['store_id'];
				$this->session->data['store_city'] = $store;
				setcookie('store_id', $query->row['store_id'], time() + 60 * 60 * 24 * 30, '/', $this->request->server['HTTP_HOST']);
			} else {
				$store = '';
			}
		}
...
if (isset($this->session->data['store_city'])) {
	...
	$this->config->set('config_city', $store);
}
...
//		}
		...
		if(isset($this->request->get['_route_'])) {
			...
			if(array_key_exists($tokens[0], $languages)) {
				$code = $tokens[0];
				$this->request->get['_route_'] = substr($this->request->get['_route_'], strlen($code) + 1);
			}
			$_route = trim($this->request->get['_route_']);
			if ($_route  == '' || $_route == 'index.php') {
